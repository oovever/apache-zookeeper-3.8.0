...
public class ClientCnxn {
    ...
    /**
     * These are the packets that have been sent and are waiting for a response. 等待响应的队列 FIFO
     * 非ping和auth发送的packet保存到此队列，收到服务端响应后，会从队列中按FIFO顺序取出Packet处理；发送后Packet添加到这个队列
     */
    private final Queue<Packet> pendingQueue = new ArrayDeque<>();

    /**
     * These are the packets that need to be sent. 发送队列 FIFO;发送前添加到这个队列
     * SendThread轮询队列中的packet进行处理
     */
    private final LinkedBlockingDeque<Packet> outgoingQueue = new LinkedBlockingDeque<Packet>();

    private int connectTimeout; //初始为sessionTimeout，后续变更为negotiatedSessionTimeout

    /**
     * The timeout in ms the client negotiated with the server. This is the
     * "real" timeout, not the timeout request by the client (which may have
     * been increased/decreased by the server which applies bounds to this
     * value. 服务端tickTime与客户端sessionTimeout计算后的session过期时间
     */
    private volatile int negotiatedSessionTimeout;

    private int readTimeout; // 读取超时，（sessionTimeout/negotiatedSessionTimeout) * 2 / 3

    private final int sessionTimeout; //自定义超时时间，不是最终的

    private final ZKWatchManager watchManager; //watcher管理器
    ...
    final SendThread sendThread; // 处理发送接收消息

    final EventThread eventThread; // 处理响应事件
    ...
    static class Packet {
        ...
        public void createBB() {
            try {
                ...
                this.bb.rewind(); // 将position置0，清除mark
            } catch (IOException e) {
...
//    需要处理的事件集合
        ...
        private void queueEvent(WatchedEvent event, Set<Watcher> materializedWatchers) {
            if (event.getType() == EventType.None && sessionState == event.getState()) { //event事件类型为空，且session状态未改变直接返回
                return;
            }
            sessionState = event.getState(); //获取当前事件的状态
            ...
            WatcherSetEventPair pair = new WatcherSetEventPair(watchers, event); //使用事件与ClientWatchManager新建对象；可能有多个相同的事件，不同session
            // queue the pair (watch set & event) for later processing
            waitingEvents.add(pair); // watch阻塞队列事件
        }
        ...
        public void queuePacket(Packet packet) {
            if (wasKilled) { // EventThread 在接收到 eventOfDeath 后 wasKilled 将被设为 true
                synchronized (waitingEvents) {
                    if (isRunning) {
                        waitingEvents.add(packet);  // 事件中添加packet
                    } else {
                        processEvent(packet); // 不在运行，处理事件
                    }
                }
            } else {
                waitingEvents.add(packet); // 事件中添加packet
        ...
        public void run() {
            try {
                isRunning = true;
                while (true) {
                    Object event = waitingEvents.take(); // 阻塞事件是否到达
                    if (event == eventOfDeath) { // 关闭事件
                        wasKilled = true;
                    } else {
                        processEvent(event); // 处理事件
                    }
                    if (wasKilled) { // 关闭时间等待waitingEvents全部处理之后再设置eventThread为停止
                        synchronized (waitingEvents) {
        ...
        private void processEvent(Object event) {
            try {
                if (event instanceof WatcherSetEventPair) {
                    ...
                    WatcherSetEventPair pair = (WatcherSetEventPair) event; //获取WatcherSetEventPair
                    for (Watcher watcher : pair.watchers) {
                        try {
                            watcher.process(pair.event); // 回调watcher
                        } catch (Throwable t) {
                    ...
                    if (p.cb == null) {
                        ...
                        if (rc == Code.OK.intValue()) {
                            ...
                            cb.processResult(rc, clientPath, p.ctx, null); // 执行方法回调
                        }
    ...
    // @VisibleForTesting 收到服务端请求执行
    protected void finishPacket(Packet p) {
        int err = p.replyHeader.getErr();
        if (p.watchRegistration != null) {
            p.watchRegistration.register(err);  // 注册watcher到队列中
        }
        ...
        if (p.cb == null) { // 不存在回调方法，同步调用
            synchronized (p) {
                p.finished = true;
                p.notifyAll(); // 唤醒创建时的wait等待
            }
        } else {
            p.finished = true;
            eventThread.queuePacket(p); // 异步处理
        }
    }
    ...
    class SendThread extends ZooKeeperThread {
        ...
        private final ClientCnxnSocket clientCnxnSocket; //socket处理对象 nio/netty
        private boolean isFirstConnect = true; //是否第一次连接
        ...
        void readResponse(ByteBuffer incomingBuffer) throws IOException {
            ...
            case PING_XID: // 处理ping请求，不做任何处理
              ...
              return;
            case NOTIFICATION_XID:  // watcher监听处理
                LOG.debug("Got notification session id: 0x{}",
                    Long.toHexString(sessionId));
                WatcherEvent event = new WatcherEvent();
                event.deserialize(bbia, "response"); //获取event事件
                ...
                WatchedEvent we = new WatchedEvent(event); //创建WatchedEvent对象
                LOG.debug("Got {} for session id 0x{}", we, Long.toHexString(sessionId));
                eventThread.queueEvent(we); // 数据变动后，添加event事件
                return;
            default:
                break;
            }
             ...
             */
            try {
                if (packet.requestHeader.getXid() != replyHdr.getXid()) { // 发送的请求与接收到的响应xid不同报错
                ...
                if (replyHdr.getZxid() > 0) {
                    lastZxid = replyHdr.getZxid(); // 更新zxid
                }
                if (packet.response != null && replyHdr.getErr() == 0) {
                    packet.response.deserialize(bbia, "response"); // 反序列化response数据
                }
        ...
        void primeConnection() throws IOException {
            ...
            isFirstConnect = false; //客户端与服务端连接完毕,设置第一次连接为false
            long sessId = (seenRwServerBefore) ? sessionId : 0;
            ConnectRequest conReq = new ConnectRequest(0, lastZxid, sessionTimeout, sessId, sessionPasswd);
            // We add backwards since we are pushing into the front watcher监听事件处理
                ...
                if (!dataWatches.isEmpty() || !existWatches.isEmpty() || !childWatches.isEmpty()
                    ...
                    while (dataWatchesIter.hasNext() || existWatchesIter.hasNext() || childWatchesIter.hasNext()
                        ...
                        if (persistentWatchesBatch.isEmpty() && persistentRecursiveWatchesBatch.isEmpty()) {
                            ...
                            record = new SetWatches(setWatchesLastZxid, dataWatchesBatch, existWatchesBatch, childWatchesBatch); // 监听封装为SetWatcher对象
                        ...
                        outgoingQueue.addFirst(packet); // 向服务端发送客户端管理的watcher
                    }
                }
            }
            ...
            outgoingQueue.addFirst(new Packet(null, null, conReq, null, null, readOnly));//添加packet数据到outgoingQueue中
            clientCnxnSocket.connectionPrimed(); //监听可写事件nio；netty空实现
        ...
        private void sendPing() {
            ...
            queuePacket(h, null, null, null, null, null, null, null, null); // ping请求只有header 其他为空
//            packet放到outgoingQueue队列中
        }
        ...
        private void startConnect(InetSocketAddress addr) throws IOException {
            ...
            changeZkState(States.CONNECTING); // 修改状态

            String hostPort = addr.getHostString() + ":" + addr.getPort();
            MDC.put("myid", hostPort);
            setName(getName().replaceAll("\\(.*\\)", "(" + hostPort + ")")); // 设置连接名称
            ...
            logStartConnect(addr); // 记录连接log日志

            clientCnxnSocket.connect(addr); // 执行连接处理 netty/nio
        }
        ...
        @Override
        public void run() {
            clientCnxnSocket.introduce(this, sessionId, outgoingQueue); // 设置sessionId与outgoingQueue队列
            clientCnxnSocket.updateNow();
            clientCnxnSocket.updateLastSendAndHeard();
            int to; // ping时间间隔
            long lastPingRwServer = Time.currentElapsedTime();
            final int MAX_SEND_PING_INTERVAL = 10000; //10 seconds //默认时间间隔
            InetSocketAddress serverAddress = null;
            while (state.isAlive()) { //连接未关闭 循环
                try {
                    if (!clientCnxnSocket.isConnected()) { //是否已连接，初始未连接，执行此处逻辑
                        // don't re-establish connection if we are closing
                        if (closing) { // zk关闭，退出
                            break;
                        }
                        if (rwServerAddress != null) { // ping时会记录server地址
                            serverAddress = rwServerAddress;
                            rwServerAddress = null;
                        } else {
                            serverAddress = hostProvider.next(1000); // 获取连接地址，所有主机都连接失败，暂停1000ms
                        }
                        onConnecting(serverAddress); // 标注正在连接的状态
                        startConnect(serverAddress); // 建立连接 连接失败后获取下一个地址继续连接
                        // Update now to start the connection timer right after we make a connection attempt
                        clientCnxnSocket.updateNow();
                        clientCnxnSocket.updateLastSendAndHeard(); // 更新连接/读写请求时间戳
                    }
                    ...
                    if (state.isConnected()) {
                        ...
                        to = readTimeout - clientCnxnSocket.getIdleRecv(); // 读时间间隔 getIdleRecv获取最后一次监听到服务端响应的时间
                    } else {
                        to = connectTimeout - clientCnxnSocket.getIdleRecv(); // 连接时间间隔
                    }

                    if (to <= 0) { // 读时间间隔不合法报异常、过期时间
                    ...
                    if (state.isConnected()) {
                        ...
                        int timeToNextPing = readTimeout / 2
                                             ...
                                             - ((clientCnxnSocket.getIdleSend() > 1000) ? 1000 : 0); //获取下次ping的时间间隔 最后发送到现在超过1000ms则减1000，不超过无操作
                        //send a ping request either time is due or no packet sent out within MAX_SEND_PING_INTERVAL timeToNextPing<0
                        if (timeToNextPing <= 0 || clientCnxnSocket.getIdleSend() > MAX_SEND_PING_INTERVAL) {
                            sendPing(); // 发送ping
                            clientCnxnSocket.updateLastSend(); // 更新最后一次发送时间
                        } else {
...
//                   处理队列中的读写事件
        ...
        void onConnected(
            ...
            negotiatedSessionTimeout = _negotiatedSessionTimeout; //client与server协商后的session过期时间
            if (negotiatedSessionTimeout <= 0) { // 小于0连接失败
                changeZkState(States.CLOSED); // 更新状态

                eventThread.queueEvent(new WatchedEvent(Watcher.Event.EventType.None, Watcher.Event.KeeperState.Expired, null)); //添加过期时间到eventThread
                eventThread.queueEventOfDeath(); // 停止服务
            ...
            readTimeout = negotiatedSessionTimeout * 2 / 3; // 读超时
            ...
            eventThread.queueEvent(new WatchedEvent(Watcher.Event.EventType.None, eventState, null)); //发送连接状态
        }
    ...
    public ReplyHeader submitRequest(
        ...
        Packet packet = queuePacket(
            ...
            null,
            watchRegistration,
            watchDeregistration); // 请求入队
        synchronized (packet) {
            if (requestTimeout > 0) {
                // Wait for request completion with timeout
                waitForPacketFinish(r, packet); // 超时等待
            } else {
    ...
    public Packet queuePacket(
        ...
        synchronized (state) {
            if (!state.isAlive() || closing) { // 连接关闭或者正在关闭
                ...
                outgoingQueue.add(packet); // packet放到outgoingQueue队列中
            }
        }
        sendThread.getClientCnxnSocket().packetAdded(); // 唤醒发送线程针对NIO，Netty空实现
        return packet;
    }
