...
public class Observer extends Learner {
    ...
    void observeLeader() throws Exception {
        ...
        try {
            self.setZabState(QuorumPeer.ZabState.DISCOVERY); // 设置状态
            QuorumServer master = findLearnerMaster(); // 寻找leader信息
            try {
                connectToLeader(master.addr, master.hostname); // 连接leader
                connectTime = System.currentTimeMillis();
                long newLeaderZxid = registerWithLeader(Leader.OBSERVERINFO); // 注册leader
    ...
    private QuorumServer findLearnerMaster() {
        ...
        final QuorumPeer.QuorumServer master = (prescribedLearnerMaster == null)
            ...
            : prescribedLearnerMaster; // 寻找leader节点
