...
public class WatchManager implements IWatchManager {
    ...
    private final Map<String, Set<Watcher>> watchTable = new HashMap<>(); // key path value watcher对象

    private final Map<Watcher, Set<String>> watch2Paths = new HashMap<>(); // key watch value watcher关联的节点
    ...
    public synchronized boolean addWatch(String path, Watcher watcher, WatcherMode watcherMode) {
        ...
        Set<Watcher> list = watchTable.get(path); // 获取path的所有watcher
        if (list == null) {
            // don't waste memory if there are few watches on a node
            // rehash when the 4th entry is added, doubling size thereafter
            // seems like a good compromise
            list = new HashSet<>(4);
            watchTable.put(path, list); // 记录path的watcher
        }
        list.add(watcher);

        Set<String> paths = watch2Paths.get(watcher); // key watcher value pathList
        if (paths == null) {
         ...
         *    STANDARD 普通模式,
         *     PERSISTENT 持久化订阅,
         *     PERSISTENT_RECURSIVE 递归订阅，针对监听的节点的子节点的变化都会触发监听
         */
    ...
    public WatcherOrBitSet triggerWatch(String path, EventType type, WatcherOrBitSet supress) {
        WatchedEvent e = new WatchedEvent(type, KeeperState.SyncConnected, path); // 根据事件类型，连接状态，和节点路径创建WatchedEvent
        Set<Watcher> watchers = new HashSet<>(); // 创建Watcher集合
        ...
        synchronized (this) {
            for (String localPath : pathParentIterator.asIterable()) {
                ...
                while (iterator.hasNext()) {
                    ...
                    if (watcherMode.isRecursive()) { // 递归watcher
                        if (type != EventType.NodeChildrenChanged) {
                            watchers.add(watcher);
                        }
                    } else if (!pathParentIterator.atParentPath()) {
                        watchers.add(watcher);
                        if (!watcherMode.isPersistent()) { // 非持久化状态移除
                            iterator.remove();
