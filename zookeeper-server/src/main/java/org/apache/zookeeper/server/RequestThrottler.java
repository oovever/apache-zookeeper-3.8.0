...
public class RequestThrottler extends ZooKeeperCriticalThread {
    ...
    public void run() {
        try {
            while (true) {
                ...
                Request request = submittedRequests.take(); //通过队列中获取请求
                ...
                if (maxRequests > 0) {
                    while (!killed) {
                        ...
                        if (zks.getInProcess() < maxRequests) { //没有超过最大请求
                            break;
                        }
                        throttleSleep(stallTime); // 限流处理
                    }
                }
                ...
                if (request != null) {
                    ...
                    zks.submitRequestNow(request); //责任链方式执行请求
                }
            }
    ...
    public void submitRequest(Request request) {
        if (stopping) {
            ...
            submittedRequests.add(request); //请求放到submittedRequests队列中
        }
    }
