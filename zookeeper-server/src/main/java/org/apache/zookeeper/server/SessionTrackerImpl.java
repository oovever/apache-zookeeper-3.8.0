...
//    保存sessionId和session对象映射关系
    protected final ConcurrentHashMap<Long, SessionImpl> sessionsById = new ConcurrentHashMap<Long, SessionImpl>();

    private final ExpiryQueue<SessionImpl> sessionExpiryQueue; //session过期队列
//  保存sessionId和session的过期时间
    protected final ConcurrentMap<Long, Integer> sessionsWithTimeout;
    private final AtomicLong nextSessionId = new AtomicLong(); //新sessionId
    ...
    public SessionTrackerImpl(SessionExpirer expirer, ConcurrentMap<Long, Integer> sessionsWithTimeout, int tickTime, long serverId, ZooKeeperServerListener listener) {
        ...
        this.sessionExpiryQueue = new ExpiryQueue<SessionImpl>(tickTime); // 会话过期队列，sessionId与session过期时间map
        this.sessionsWithTimeout = sessionsWithTimeout; // 保存sessionId和session的过期时间
        this.nextSessionId.set(initializeNextSessionId(serverId));
        for (Entry<Long, Integer> e : sessionsWithTimeout.entrySet()) { //加载内存中的session保存
            trackSession(e.getKey(), e.getValue()); //sessionsWithTimeout key sessionId value 会话过期时间
        }
    ...
    public void run() {
        try {
            while (running) {
                long waitTime = sessionExpiryQueue.getWaitTime(); //获取当前时间到清理时间点的差值
                if (waitTime > 0) {
                    Thread.sleep(waitTime); //等待时间大于0，未到清理时间点 wait
                    continue;
                }
//               获取过期队列中的数据，关闭session连接
    ...
    private void updateSessionExpiry(SessionImpl s, int timeout) {
        ...
        sessionExpiryQueue.update(s, timeout); //更新session过期时间
    }
...
//    创建session
    public long createSession(int sessionTimeout) {
        ...
        trackSession(sessionId, sessionTimeout); // 创建session
        return sessionId;
    }
    ...
    public synchronized boolean trackSession(long id, int sessionTimeout) {
        ...
        SessionImpl session = sessionsById.get(id); //没有session对象则新建
        if (session == null) {
        ...
        updateSessionExpiry(session, sessionTimeout); //更新session过期时间
        return added;
    }
