...
public class FastLeaderElection implements Election {
     ...
     * the system up again after long partitions. Currently 60 seconds.达到maxNotificationInterval选举未成功将重启机器
     */
    ...
    LinkedBlockingQueue<ToSend> sendqueue; // 消息发送队列
    LinkedBlockingQueue<Notification> recvqueue; // 消息接受对列
        ...
        class WorkerReceiver extends ZooKeeperThread {
            ...
            public void run() {
                ...
                while (!stop) {
                    // Sleeps on receive
                    try {
                        response = manager.pollRecvQueue(3000, TimeUnit.MILLISECONDS); //通过recvQueue获取RecvWorker对象
                        if (response == null) {
                            continue;
                        }
                         ...
                         */
                        if (!validVoter(response.sid)) { //参与选举的机器不包含请求的机器
                            ...
                            sendqueue.offer(notmsg); // 给远程请求机器回消息，告知当前集群的leader信息，远程机器根据集群leader信息变为follower或者observer
                        } else {
                            // Receive new message
...
//                            获取响应中的对象信息,消息可能是发送方认为的leader机器，myId2发送的选票 可能是myId3的信息；
                            n.leader = rleader;
                            n.zxid = rzxid;
                            ...
                            if (self.getPeerState() == QuorumPeer.ServerState.LOOKING) { // 当前机器处于选举状态
                                recvqueue.offer(n); //入队
                                 ...
                                 * lagging behind.
                                 */
                                if ((ackstate == QuorumPeer.ServerState.LOOKING) //如果收到的消息的机器处于Looking状态
                                    ...
                                    sendqueue.offer(notmsg); //发送当前机器认为的leader及当前选票信息
                                }
                            } else {
                                /*
                                 ...
                                 */
                                Vote current = self.getCurrentVote(); //处理中途加入的服务，当前节点已选举完成
                                if (ackstate == QuorumPeer.ServerState.LOOKING) { //如果是Looking状态
                                    if (self.leader != null) { // 如果当前节点是leader
                                    ...
                                    ToSend notmsg = new ToSend(
                                        ...
                                        current.getPeerEpoch(),
                                        qv.toString().getBytes());
                                    sendqueue.offer(notmsg); //向对方机器发送消息
                                }
                            }
                        }
        ...
        class WorkerSender extends ZooKeeperThread {
            ...
            volatile boolean stop;
            QuorumCnxManager manager; //连接管理对象
            ...
            public void run() {
                while (!stop) {
                    try {
                        ToSend m = sendqueue.poll(3000, TimeUnit.MILLISECONDS); //获取sendQueue中的数据选票
                        if (m == null) {
                            continue;
                        }
//                      处理发送的选票
            ...
            void process(ToSend m) {
                ByteBuffer requestBuffer = buildMsg(m.state.ordinal(), m.leader, m.zxid, m.electionEpoch, m.peerEpoch, m.configData); // 转化为ByteBuffer对象
...
//      接受选票线程
         ...
         */
        void start() {
            this.wsThread.start(); // 发送线程
            this.wrThread.start(); // 接收线程
        }
    ...
    QuorumPeer self;
    Messenger messenger;
    AtomicLong logicalclock = new AtomicLong(); /* Election instance */ //选举周期，每次选举加1，electionEpoch属性
    long proposedLeader; // 选票leader myid
    long proposedZxid; // leader的zxid
    long proposedEpoch; // 选举周期,每次选举最终确定完leader结束选举流程时会自增(真正zxid的前32位)
...
//       向所有其他vote机器发送选票 getCurrentAndNextConfigVoters myId set
        for (long sid : self.getCurrentAndNextConfigVoters()) {
            QuorumVerifier qv = self.getQuorumVerifier(); // 发送选票数据
            ToSend notmsg = new ToSend(
                ToSend.mType.notification,
                proposedLeader,  //第一次是自己 ，要选举的leader
                proposedZxid,   // zxid 最大事务ID
                logicalclock.get(), // 当前选举周期
                QuorumPeer.ServerState.LOOKING, //当前节点状态
                sid, //目标机器 myid
                proposedEpoch, // epoch// leader 选票epoch
                qv.toString().getBytes(UTF_8)); //构建选票
...
//            消息放到内存队列中
    ...
    protected boolean totalOrderPredicate(long newId, long newZxid, long newEpoch, long curId, long curZxid, long curEpoch) {
        ...
        return ((newEpoch > curEpoch)
                || ((newEpoch == curEpoch)
                    && ((newZxid > curZxid)
                        || ((newZxid == curZxid)
                            && (newId > curId))))); // 1. 选举周期 2. zxid 3. myid
    }
    ...
    protected SyncedLearnerTracker getVoteTracker(Map<Long, Vote> votes, Vote vote) {
        ...
        for (Map.Entry<Long, Vote> entry : votes.entrySet()) {
            if (vote.equals(entry.getValue())) {
                voteSet.addAck(entry.getKey()); // 选票存储到voteSet中
            }
        }
    ...
    protected boolean checkLeader(Map<Long, Vote> votes, long leader, long electionEpoch) {
        ...
        if (leader != self.getId()) { // 自己不是Leader
            if (votes.get(leader) == null) {  // 投票过程中未收到leader记录
                predicate = false;
            } else if (votes.get(leader).getState() != ServerState.LEADING) { // 当前选票中，leader不知道自己的状态
                predicate = false;
            }
        } else if (logicalclock.get() != electionEpoch) { // 自己是leader但是选举周期不同
            predicate = false;
        }
...
//       选举的节点是否包含当前节点
    ...
    public Vote lookForLeader() throws InterruptedException {
        ...
        try {
             ...
             * the electionEpoch of the received notifications) in a leader election. 存储非法选票
             */
            Map<Long, Vote> outofelection = new HashMap<Long, Vote>();

            int notTimeout = minNotificationInterval; //轮询其他机器发送消息的间隔时间

            synchronized (this) {
                logicalclock.incrementAndGet(); // 增加选举周期
//              更新选票,myId zxid epoch getInitLastLoggedZxid获取最大zxid
                updateProposal(getInitId(), getInitLastLoggedZxid(), getPeerEpoch()); // 初始化选票
            }
...
//            发送选票 选票会给自己发
            ...
            while ((self.getPeerState() == ServerState.LOOKING) && (!stop)) {
                 ...
                 * Otherwise processes new notification.收不到选票 开始时收不到选票
                 */
                if (n == null) {
                    if (manager.haveDelivered()) { //队列中是否有选票未发送
                        sendNotifications();
                    } else {
                        manager.connectAll(); //第一次启动执行 与其他节点建立Socket连接 启动后相互连接;连接失败，在这里一直重试
                    }

                    /*
                     * Exponential backoff 轮询时间翻倍
                     */
...
//                    默认返回false
                ...
                } else if (validVoter(n.sid) && validVoter(n.leader)) { // 校验选票是否有效
                    /*
                     * Only proceed if the vote comes from a replica in the current or next
                     * voting view for a replica in the current or next voting view.
                     */
                    switch (n.state) {
                    case LOOKING: //收到消息状态为选举中
                        ...
                        if (n.electionEpoch > logicalclock.get()) { //判断选举周期是否大于当前本地的选举周期，选举时logicalclock会加1
                            logicalclock.set(n.electionEpoch); //选举过程中要保证机器的electionEpoch一致
                            recvset.clear();//清空当前服务接收到其他机器的选票情况
                            if (totalOrderPredicate(n.leader, n.zxid, n.peerEpoch, getInitId(), getInitLastLoggedZxid(), getPeerEpoch())) {
                                updateProposal(n.leader, n.zxid, n.peerEpoch); //对端发送机器胜出，更新选票 peerEpoch选举leader对应的选举周期
                            } else {
                                updateProposal(getInitId(), getInitLastLoggedZxid(), getPeerEpoch()); //本机胜出，修改投票信息为本机信息
                            }
                            sendNotifications(); //发送投票信息
                                ...
                                LOG.debug(
                                    ...
                                    Long.toHexString(logicalclock.get()));
                            break;
                        } else if (totalOrderPredicate(n.leader, n.zxid, n.peerEpoch, proposedLeader, proposedZxid, proposedEpoch)) { // pk leader逻辑
                            updateProposal(n.leader, n.zxid, n.peerEpoch); // 更新最新选票，此处远程选票大于本机选票
                        ...
                        // don't care about the version if it's in LOOKING state 获取所有收到的选票 计算选票是否超过半数使用 本机记录选票信息
                        recvset.put(n.sid, new Vote(n.leader, n.zxid, n.electionEpoch, n.peerEpoch));
//                       创建选票跟踪器
                        voteSet = getVoteTracker(recvset, new Vote(proposedLeader, proposedZxid, logicalclock.get(), proposedEpoch)); // 计算选票是否会超过半数，已收到的选票与当前将要投票信息

                        if (voteSet.hasAllQuorums()) { //超过一半选票

                            // Verify if there is any change in the proposed leader 通过接收队列获取是否有新的选票，如果有新的选票 重新走计算逻辑
                            while ((n = recvqueue.poll(finalizeWait, TimeUnit.MILLISECONDS)) != null) {
                                if (totalOrderPredicate(n.leader, n.zxid, n.peerEpoch, proposedLeader, proposedZxid, proposedEpoch)) { //后续机器更适合当leader
                                    recvqueue.put(n);
                                    break;
                                }
                            }
                             ...
                             */
                            if (n == null) { // n==null表示没有更适合的leader
                                setPeerState(proposedLeader, voteSet); // 判断当前选票是否自己，如果是自己设置leader,否则设置为follower状态
                                Vote endVote = new Vote(proposedLeader, proposedZxid, logicalclock.get(), proposedEpoch); // 新建leader选票
                                leaveInstance(endVote); //打印ledaer信息并清空recvqueue
                                return endVote;
                            }
                        }
                        break;
                    case OBSERVING: //收到observer通知
    ...
    private Vote receivedFollowingNotification(Map<Long, Vote> recvset, Map<Long, Vote> outofelection, SyncedLearnerTracker voteSet, Notification n) {
         ...
         * together.
         */
        if (n.electionEpoch == logicalclock.get()) { // 比较选举周期和当前选举周期是否相等
            recvset.put(n.sid, new Vote(n.leader, n.zxid, n.electionEpoch, n.peerEpoch, n.state));
            voteSet = getVoteTracker(recvset, new Vote(n.version, n.leader, n.zxid, n.electionEpoch, n.peerEpoch, n.state));
            if (voteSet.hasAllQuorums() && checkLeader(recvset, n.leader, n.electionEpoch)) { // 检查follower机器返回的leader状态
                setPeerState(n.leader, voteSet); // 设置leader状态
                Vote endVote = new Vote(n.leader, n.zxid, n.electionEpoch, n.peerEpoch); // 更新最终选票
         ...
         */
        outofelection.put(n.sid, new Vote(n.version, n.leader, n.zxid, n.electionEpoch, n.peerEpoch, n.state)); //记录选票状态
        voteSet = getVoteTracker(outofelection, new Vote(n.version, n.leader, n.zxid, n.electionEpoch, n.peerEpoch, n.state));

        if (voteSet.hasAllQuorums() && checkLeader(outofelection, n.leader, n.electionEpoch)) {
            synchronized (this) { // PK leader选举 设置当前Following状态
