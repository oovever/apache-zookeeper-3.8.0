...
public class Follower extends Learner {
    ...
    void followLeader() throws InterruptedException {
        ...
        try {
            ...
            QuorumServer leaderServer = findLeader(); // 通过集群投票信息，寻找选举的leader
            try {
                connectToLeader(leaderServer.addr, leaderServer.hostname); //与leader建立socket通道 同步leader中的数据
                connectionTime = System.currentTimeMillis();
                long newEpochZxid = registerWithLeader(Leader.FOLLOWERINFO); // 注册follower到leader并同步leader的信息并获取最新的zxid信息
                if (self.isReconfigStateChange()) {
                    throw new Exception("learned about role change");
                }
                //check to see if the leader zxid is lower than ours
                //this should never happen but is just a safety check
                long newEpoch = ZxidUtils.getEpochFromZxid(newEpochZxid); // 获取最新epoch
                if (newEpoch < self.getAcceptedEpoch()) { // leader epoch小于当前epoch抛出错误，不可能出现这种情况
                ...
                syncWithLeader(newEpochZxid); // 通过leader中获取数据，同步leader中的数据
                ...
                while (this.isRunning()) { // 获取leader节点中的数据
                    readPacket(qp); // 读取包数据
                    processPacket(qp); // 处理包数据
                }
            } catch (Exception e) {
                LOG.warn("Exception when following the leader", e); // socket无法接受到leader数据
    ...
    protected void processPacket(QuorumPacket qp) throws Exception {
        ...
        case Leader.PROPOSAL: // 处理数据请求
            ...
            fzk.logRequest(hdr, txn, digest); //记录到日志文件中
            if (hdr != null) {
                /*
