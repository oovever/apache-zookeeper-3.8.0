...
public class SyncRequestProcessor extends ZooKeeperCriticalThread implements RequestProcessor {
    ...
    private final BlockingQueue<Request> queuedRequests = new LinkedBlockingQueue<Request>(); // 需要处理的请求
     ...
     */
    private final Queue<Request> toFlush; //等待刷入磁盘的请求
    ...
    public void run() {
        try {
            ...
            resetSnapshotStats(); // 重置randSize和randRoll，使得每次生成快照的时机不一样
            lastFlushTime = Time.currentElapsedTime();
            while (true) {
                ServerMetrics.getMetrics().SYNC_PROCESSOR_QUEUE_SIZE.add(queuedRequests.size());

                long pollTime = Math.min(zks.getMaxWriteQueuePollTime(), getRemainingDelay());
                Request si = queuedRequests.poll(pollTime, TimeUnit.MILLISECONDS); //获取请求
                if (si == null) {
                    /* We timed out looking for more writes to batch, go ahead and flush immediately */
                    flush();
                    si = queuedRequests.take(); // 获取队列中的请求
                }
                ...
                // track the number of records written to the log // si写入到database中
                if (!si.isThrottled() && zks.getZKDatabase().append(si)) {
                    if (shouldSnapshot()) { //快照处理
                        resetSnapshotStats();
                        // roll the log
                        zks.getZKDatabase().rollLog(); // 更换新文件存储日志
                        // take a snapshot
                    ...
                    if (nextProcessor != null) {
                        nextProcessor.processRequest(si); //所有任务处理完成调用下一个processor
                ...
                toFlush.add(si);
                if (shouldFlush()) { //toFlush队列中数据大于配置flush阈值 flush一次
                    flush();
                }
    ...
    public void processRequest(final Request request) {
        ...
        queuedRequests.add(request); // 向队列中添加数据
