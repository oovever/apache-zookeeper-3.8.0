...
public class DataTree {
    ...
    private IWatchManager dataWatches; // 节点watch

    private IWatchManager childWatches; // 子节点watch
    ...
    /**
     * This hashtable lists the paths of the ephemeral nodes of a session. 临时节点
     */
    ...
    private final ReferenceCountedACLCache aclCache = new ReferenceCountedACLCache(); // acl控制
    ...
    public void createNode(final String path, byte[] data, List<ACL> acl, long ephemeralOwner, int parentCVersion, long zxid, long time, Stat outputStat) throws KeeperException.NoNodeException, KeeperException.NodeExistsException {
        ...
        dataWatches.triggerWatch(path, Event.EventType.NodeCreated); // 触发watch的创建事件
        childWatches.triggerWatch(parentName.equals("") ? "/" : parentName, Event.EventType.NodeChildrenChanged); // 触发子节点变更
    }
    ...
    public byte[] getData(String path, Stat stat, Watcher watcher) throws KeeperException.NoNodeException {
        ...
        synchronized (n) {
            n.copyStat(stat);
            if (watcher != null) {
                dataWatches.addWatch(path, watcher); // 监听事件放到watchTable中
            }
            data = n.data;
        }
