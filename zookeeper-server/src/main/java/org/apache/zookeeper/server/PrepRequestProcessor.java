...
public class PrepRequestProcessor extends ZooKeeperCriticalThread implements RequestProcessor {
    ...
    LinkedBlockingQueue<Request> submittedRequests = new LinkedBlockingQueue<Request>(); //保存的要处理的请求

    private final RequestProcessor nextProcessor; //责任链下一个processor
    ...
    public void run() {
        ...
        try {
            while (true) {
                ...
                Request request = submittedRequests.take(); //获取阻塞队列中的请求 责任链放进去的
                ...
                if (Request.requestOfDeath == request) { // ZK已经关闭，因此退出循环
                    break;
                }
    ...
    protected void pRequest2Txn(int type, long zxid, Request request, Record record, boolean deserialize) throws KeeperException, IOException, RequestProcessorException {
        if (request.getHdr() == null) {
            request.setHdr(new TxnHeader(request.sessionId, request.cxid, zxid,
                    Time.currentWallTime(), type)); //设置请求头
        }
        ...
        case OpCode.createContainer: {
            pRequest2TxnCreate(type, request, record, deserialize); //创建请求处理
            break;
        }
        ...
        case OpCode.createSession:
            ...
            int to = request.request.getInt(); //获取sessionTimeOut
            request.setTxn(new CreateSessionTxn(to)); // 创建SessionTxn对象
            request.request.rewind();
    ...
    protected void pRequest(Request request) throws RequestProcessorException {
        ...
        if (!request.isThrottled()) {
          pRequestHelper(request); // 处理request请求 根据不同命令create delete 等调用不同处理方法
        }
        ...
        nextProcessor.processRequest(request); // 执行下一个proposal
    ...
    private void pRequestHelper(Request request) throws RequestProcessorException {
        try {
            ...
            case OpCode.create:
            case OpCode.create2:
                CreateRequest create2Request = new CreateRequest(); // 创建CreateRequest
            ...
            case OpCode.whoAmI:
                zks.sessionTracker.checkSession(request.sessionId, request.getOwner()); //判断sessionId和对应session是否相同
                break;
            default:
    ...
    public void processRequest(Request request) {
        ...
        submittedRequests.add(request); //添加请求到队列中，异步处理
