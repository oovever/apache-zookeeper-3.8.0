...
public class FileTxnSnapLog {
    ...
    public long restore(DataTree dt, Map<Long, Integer> sessions, PlayBackListener listener) throws IOException {
        ...
        long deserializeResult = snapLog.deserialize(dt, sessions); // 从快照数据中反序列化数据
        ...
        RestoreFinalizer finalizer = () -> {
            long highestZxid = fastForwardFromEdits(dt, sessions, listener); //加载事务日志（快照id-》当前id） 获取最新zxid
            // The snapshotZxidDigest will reset after replaying the txn of the
            // zxid in the snapshotZxidDigest, if it's not reset to null after
            // restoring, it means either there are not enough txns to cover that
            // zxid or that txn is missing
            DataTree.ZxidDigest snapshotZxidDigest = dt.getDigestFromLoadedSnapshot();
            if (snapshotZxidDigest != null) { //快照zxid和日志zxid不一致
                LOG.warn(
...
//        当前没快照
    ...
    public long fastForwardFromEdits(
        ...
        TxnIterator itr = txnLog.read(dt.lastProcessedZxid + 1); //从lastProcessedZxid读日志文件
        ...
        try {
            while (true) {
                ...
                try {
                    processTransaction(hdr, dt, sessions, itr.getTxn()); //处理session连接
    ...
    public void processTransaction(
        ...
        ProcessTxnResult rc;
        switch (hdr.getType()) {
        case OpCode.createSession: // 处理新连接
            sessions.put(hdr.getClientId(), ((CreateSessionTxn) txn).getTimeOut()); //注册新连接
        ...
        case OpCode.closeSession:
            sessions.remove(hdr.getClientId()); //关闭连接删除session
            if (LOG.isTraceEnabled()) {
                ZooTrace.logTraceMessage(
                    LOG,
                    ZooTrace.SESSION_TRACE_MASK,
            ...
            break;
        default:
            rc = dt.processTxn(hdr, txn); //处理不同连接功能
        }
