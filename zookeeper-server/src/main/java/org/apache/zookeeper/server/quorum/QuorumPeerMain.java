...
public class QuorumPeerMain {
    ...
    public static void main(String[] args) {
        ...
        try {
            main.initializeAndRun(args); //解析配置文件
    ...
    protected void initializeAndRun(String[] args) throws ConfigException, IOException, AdminServerException {
        ...
        if (args.length == 1) {
            //  获取配置文件并解析 args为config配置文件路径
            config.parse(args[0]);
        }
//        清理数据无用文件目录 定时任务清理 如快照文件等
        // Start and schedule the the purge task
        DatadirCleanupManager purgeMgr = new DatadirCleanupManager(
            config.getDataDir(),
            config.getDataLogDir(),
            config.getSnapRetainCount(), // 保留snapShot个数
            config.getPurgeInterval()); // 几小时清理一次
        purgeMgr.start(); // 开启定时线程处理
//      是否集群模式
...
//            单机启动
    ...
    public void runFromConfig(QuorumPeerConfig config) throws IOException, AdminServerException {
        try {
            ManagedUtil.registerLog4jMBeans(); //注册JMX log4j控制器
        } catch (JMException e) {
...
//            设置ZK 数据库（内存数据库）
            quorumPeer.setZKDatabase(new ZKDatabase(quorumPeer.getTxnFactory())); // 初始化zkData，初始化时赋值创建dateTree 初始化一些数据值
