...
public class ZooKeeperServerMain {
    ...
    protected void initializeAndRun(String[] args) throws ConfigException, IOException, AdminServerException {
        try {
            ManagedUtil.registerLog4jMBeans(); //注册log4j jmx
        } catch (JMException e) {
            LOG.warn("Unable to register log4j JMX control", e);
        }
//      根据地址/client host解析地址
        ServerConfig config = new ServerConfig();
        if (args.length == 1) {
            config.parse(args[0]); // cfg路径解析
        } else {
            config.parse(args); // 四元参数解析 端口地址 日志目录 心跳时间间隔 最大连接数
        }

        runFromConfig(config); // 根据配置信息启动服务端
    }
    ...
    public void runFromConfig(ServerConfig config) throws IOException, AdminServerException {
        ...
        try {
            ...
            txnLog = new FileTxnSnapLog(config.dataLogDir, config.dataDir); // 如果目录为空则创建
            JvmPauseMonitor jvmPauseMonitor = null;
            if (config.jvmPauseMonitorToRun) {
                jvmPauseMonitor = new JvmPauseMonitor(config);
            }
            final ZooKeeperServer zkServer = new ZooKeeperServer(jvmPauseMonitor, txnLog, config.tickTime, config.minSessionTimeout, config.maxSessionTimeout, config.listenBacklog, null, config.initialConfig); // 创建server对象
            ...
            // Start Admin server
            adminServer = AdminServerFactory.createAdminServer(); // 启动监控server
            adminServer.setZooKeeperServer(zkServer);
            adminServer.start();

            boolean needStartZKServer = true;
            if (config.getClientPortAddress() != null) {
                cnxnFactory = ServerCnxnFactory.createFactory(); //根据nio/netty配置创建factory对象
                cnxnFactory.configure(config.getClientPortAddress(), config.getMaxClientCnxns(), config.getClientPortListenBacklog(), false); // 初始化赋值
                cnxnFactory.startup(zkServer); // 启动服务
