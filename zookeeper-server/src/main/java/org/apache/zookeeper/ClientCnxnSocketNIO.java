...
public class ClientCnxnSocketNIO extends ClientCnxnSocket {
    ...
    void doIO(Queue<Packet> pendingQueue, ClientCnxn cnxn) throws InterruptedException, IOException {
        ...
        if (sockKey.isWritable()) {
            Packet p = findSendablePacket(outgoingQueue, sendThread.tunnelAuthInProgress()); // 通过outgoingQueue获取数据
    ...
    void connect(InetSocketAddress addr) throws IOException {
        ...
        try {
            registerAndConnect(sock, addr); //注册到selector上
    ...
    private synchronized void wakeupCnxn() {
        selector.wakeup(); //唤醒selector阻塞，唤醒selector写阻塞
    }
    ...
    void doTransport(
        ...
        selector.select(waitTimeOut); // 阻塞获取事件
        Set<SelectionKey> selected;
        synchronized (this) {
            selected = selector.selectedKeys(); //获取就绪事件
        }
...
//        根据不同事件做不同处理
        for (SelectionKey k : selected) {
            ...
            if ((k.readyOps() & SelectionKey.OP_CONNECT) != 0) {
                ...
                doIO(pendingQueue, cnxn); // 读写事件相处理
            }
        }
